var BITNEWSLETTERSUBSCRIBER = {
    ...
    showImportSubscribers: function () {
        ...
        BITNEWSLETTERSUBSCRIBER.changeDelimiterCSV();
        $(".ui-dialog-buttonpane button:contains('Volgende') span").show();
        $(".ui-dialog-buttonpane button:contains('Vorige') span").show();
    ...
    ImportSubscribers: function () {
        ...
        if ($('[name="savedefinition"]').is(':checked') == true && $('#definitionname').val() != "") {
            ...
            BITNEWSLETTERSUBSCRIBER.importDefinition.SaveDefinition = $('[name="savedefinition"]').is(':checked');
        }
        ...
        $(BITNEWSLETTERSUBSCRIBER.newsGroups).each(function () {
            ...
            BITAJAX.callWebServiceASync("ImportSubscribers", jsonstring, function (data) {
                ...
                if (data.d.Success) {
                    ...
                    $.each(data.d.DataObject.Log, function (key, value) {

                        //Mogelijk met data-fields

                        if (value.indexOf("GESLAAGD") >=0) {
                            $('#foutlog tbody').append('<tr><td>' + value+ '</td></tr>');
                            countsucces++
                        }
                        else {
                            $('#foutlog tbody').append('<tr style="color:red;"><td>' + value + '</td></tr>');
                            countfailure++
                        }
                    ...
                    $('#importProgress').html('<div>Het importeren is voltooid. <br /> Hier vindt u de log van alle gelukte en mislukte addressen.</div>  <br />' + countsucces + ' geslaagd <br /> ' + countfailure + ' fouten <br /> ' + (countsucces + countfailure) + ' totaal<br />' + '<a href="' +importLogFile + '" target="_blanc">download errorlog</a> <form action="'+importLogFile+'" ><input type="submit" value="Download Log.txt" ></form>')
        ...
        $('#bitImportSubscribersDialog form').iframePostForm({
                ...
                complete: function (data) {
                    ...
                    if (Extension == ".csv" || Extension == ".xml" || Extension == ".txt") {
                        ...
                        $("#uploadReadyMsg").html('Het uploaden is gelukt ');
                        //BITNEWSLETTERSUBSCRIBER.firstRowisName();
                        switch (Extension) {
                            case ".csv":
                        ...
                        $("#uploadReadyMsg").html(Extension + ' is niet het juiste bestands type, kan niet uploaden.');
                    }
                },
            });

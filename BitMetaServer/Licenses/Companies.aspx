<%@ Page Title="" Language="C#" MasterPageFile="~/_MasterPages/Grids.master"
    AutoEventWireup="true" CodeBehind="Companies.aspx.cs" Inherits="BitMetaServer.Licenses.Companies" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadPlaceHolder" runat="server">
    <script type="text/javascript" src="BITCOMPANIES.js"></script>
    <script type="text/javascript">
        $(document).ready(function () {
            BITCOMPANIES.initialize();
            if (location.hash) {
                var resellerid = location.hash.replace('#', '');
                BITCOMPANIES.loadCompanies(resellerid);
                $("select.selectReseller").val(resellerid);
            }
            else {
                BITCOMPANIES.loadCompanies();
            }
        });
    </script>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MenuBarPlaceHolder" runat="server">
    <ul>
        <li><a href="../Default.aspx" class="bitNavBarButtonBack" runat="server" id="BackLink">
            back</a>
            <div>Terug</div>
        </li>

        <li><a href="javascript:BITCOMPANIES.newCompany();" class="bitNavBarButtonAddPage">
            ADDCOMPANY</a>
            <div>Nieuwe klant...</div>
        </li>

    </ul>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="TitlePlaceHolder" runat="server">
    Bitplate klanten
</asp:Content>

<asp:Content ID="Content4" ContentPlaceHolderID="GridPlaceHolder" runat="server">
    <fieldset>
        <legend>Filter</legend>
        <label>Kies handelaar:</label>
        <select id="selectResellerFilter" runat="server" class="selectReseller" onchange="BITCOMPANIES.filterCompanies(this);">
        </select>

    </fieldset>
    <table id="tableCompanies" class="bitGrid" data-control-type="table" cellspacing="0"
        cellpadding="2">
        <thead>
            <tr>
                <td class="bitTableColumn" style="width: 40px">&nbsp;</td>
                <td class="bitTableColumn bitTableColumnEllipsis" style="width: 352px" data-sort-field="Name">Naam</td>
                <td class="resellerColumn" style="width: 340px" style="display: none" data-sort-field="FK_Company">
                    Handelaar</td>
                <td class="bitTableActionButton">Bewerk</td>
                <td class="bitTableActionButton">Verwijder</td>
                <td class="bitTableDateColumn" style="width: 120px;" data-sort-field="CreateDate">Aanmaakdatum
                </td>
            </tr>
        </thead>
        <tbody>
            <tr data-id-field="ID">
                <td class="icon"><a href="#" class="bitTablePageIcon"></a></td>
                <td class="nameColumn"  data-field="Name"></td>
                <td class="resellerColumn" data-field="Reseller.Name"></td>
                <td class="bitTableActionButton"><a data-field="ID" data-title-field="Type" href="javascript:BITCOMPANIES.openCompanyDetailsPopup('[data-field]', '[data-title-field]');"
                    class="bitConfigButton"></a></td>
                <td class="bitTableActionButton"><a data-field="ID" href="javascript:BITCOMPANIES.removeCompany('[data-field]');"
                    class="bitDeleteButton"></a></td>

                <td class="bitTableDateColumn" data-field="CreateDate"></td>
            </tr>
        </tbody>
        <tfoot>
            <tr>
                <td colspan="8">
                    <div class="bitEndTable"></div>
                </td>
            </tr>
        </tfoot>
    </table>
</asp:Content>
<asp:Content ID="Content5" ContentPlaceHolderID="PopupsPlaceHolder" runat="server">
    <div id="bitCompanyDetailsDialog" title="Relatie">
        <div class="bitTabs">
            <ul>
                <li><a href="#tabPageDetails1">Algemene gegevens</a></li>
                <li><a href="#tabPageDetails2">Adres gegevens</a></li>
                <li><a href="#tabPageDetails3">Email voor wachtwoord</a></li>
            </ul>
            <!-- TAB1 -->
            <!-- TAB1 -->
            <!-- TAB1 -->
            <div id="tabPageDetails1" class="bitTabPage">
                <div class="bitPageSettingsCollumnA">Naam</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="Name" type="text" data-validation="required" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Email</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="Email" type="text" data-validation="required" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Handelaar</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <select id="selectReseller" runat="server" data-field="Reseller.ID" data-text-field="Reseller.Name"
                        data-validation="required">
                    </select>
                </div>

                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Contactpersoon</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="ContactPerson" type="text" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Telefoon</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="Telephone" type="text" />
                </div>
                <br clear="all" />
                
            </div>
            <!--TAB2-->
            <!--TAB2-->
            <!--TAB2-->
            <div id="tabPageDetails2" class="bitTabPage">
                <div class="bitPageSettingsCollumnA">Adres</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="Address" type="text" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Postcode</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="Postalcode" type="text" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Plaats</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="City" type="text" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Land</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="Country" type="text" />
                </div>
                <br clear="all" />

            </div>
            <!--TAB3-->
            <!--TAB3 Email instellingen-->
            <!--TAB3-->
            <div id="tabPageDetails3" class="bitTabPage">
                <i><u><b>Instellingen voor verzenden van een email met wachtwoord</b></u> aan nieuwe bitplate-gebuikers en
                    bij het opnieuw aanvragen van een wachtwoord.
                    <br />
                    Indien niks is ingevuld neemt Bitplate de betreffende waarde over van de handelaar.</i>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Email afzender</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="PasswordEmailFrom" type="text" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">Email onderwerp</div>
                <div class="bitPageSettingsCollumnB"></div>
                <div class="bitPageSettingsCollumnC">
                    <input data-field="PasswordEmailSubject" type="text" />
                </div>
                <br clear="all" />
                <div class="bitPageSettingsCollumnA">
                    Email template<br />
                    Gebruik tags:<br />
                    [name]<br />
                    [email]<br />
                    [password]<br />
                    [url]<br />
                </div>
                <div class="bitPageSettingsCollumnB">
                    <a href="javascript:;" class="bitTextareaEnlargeButton">enlarge </a>
                </div>
                <div class="bitPageSettingsCollumnC">
                    <textarea data-field="PasswordEmailTemplate" class="bitTextArea"></textarea>
                </div>
                <br clear="all" />
            </div>
        </div>
    </div>

</asp:Content>

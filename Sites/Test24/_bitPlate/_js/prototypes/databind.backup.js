//constants
var CONTROLTYPES = { TEXTBOX: "TEXTBOX", DROPDOWN: "DROPDOWN", CHECKBOX: "CHECKBOX", RADIO: "RADIO", CHECKBOXLIST: "CHECKBOXLIST", IMAGE: "IMAGE", HYPERLINK: "HYPERLINK", TEXTAREA: "TEXTAREA", OTHERHTML: "OTHERHTML", WYSIWYG: "WYSIWYG", LIST: "LIST", TABLE: "TABLE", TABLEROW: "TABLEROW" };
//voor lists
...
(function ($) {
    $.fn.extend({
        ...
        dataBindList: function (list, options) {
            ...
            html = html.replace(/data-field/g, "data-list-field");
            //if (controlType == CONTROLTYPES.LIST) {
            //    $(this).html(html);
            //}
            //else
            if (controlType == CONTROLTYPES.TABLE) {
                ...
                $(dataControl).find('tbody').html(html);
        ...
        fillDropDown: function (list, firstValue, valuefield) {
            ...
            //clear all options
            $(this).get(0).length = 0;
            if (firstValue != undefined && typeof firstValue != 'number') {
                var id = firstValue;
                var text = firstValue;
...
function setControlValues(dataControl, controlType, dataValues) {
    ...
    if (controlType == CONTROLTYPES.CHECKBOX) {
        ...
        if (dataValue && dataValue.constructor == Date) {
            ...
            //console.log($.browser);
            if ($($.browser.webkit)) {
                format = "yyyy-MM-dd";                  //!! HTML5 datefieldformat (VERPLICHT)
            }
...
function getDataValues(dataObj, fieldNames) {
    ...
    if (dataObj.BitListIndex) {
        dataValues.listIndex = dataObj.BitListIndex;
    }
    return dataValues;
}
